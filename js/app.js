...
var dashb = new Dashboard({
    ...
    db: {
        schema: schema,
        filters: filters,
        widgetFilter:{yearMonthDay: ['BETWEENEQ (' + dateToYMD(365) + ',' + dateToYMD(0) + ')']}
...
function renderDefaultDashboard()
{
    ...
    let widgetsTemplate = [
        ...
        {
            html: '<div class="widget" style="flex-basis: 100%;">\
      <h1>Dashboard</h1>\
      </div>',
            mode: WIDGET_TEXT
        },
        {
            html: '<div class="widget section" style="flex-basis: 100%;">\
      ...
      <p>Informació relativa al número d\'interaccions</p>\
        ...
        /*Crea primer widget*/
        {
            ...
            snippet: '{let number = "%COUNT%"-0;document.getElementById("rows_%ID%").classList.add("rowsOnly");document.getElementById("rows_%ID%").innerHTML = number.toLocaleString();}',
            kpi: ''
        }, {
            height: 200,
            size: 0.5,
            title: "Tasques",
            mode: WIDGET_CODE_SNIPPET,
            snippet: '{let number = "%COUNT%"-0;document.getElementById("rows_%ID%").innerHTML = number.toLocaleString();}',
            field: "event",
            ...
            snippet: '{let number = "%COUNT%"-0;document.getElementById("rows_%ID%").innerHTML = number.toLocaleString();}',
            field: "event",
            filter: { component: ['IN (Fitxer)'], description: ['CONTAINS (\'resource\' activity)'] }
        }, {
            height: 200,
            size: 0.5,
            title: "Pàgines",
      ...
      chart.render();',
            ...
            filter: [
                ...
                { component: ['IN (Fitxer)'], description: ['CONTAINS (\'resource\' activity)'] },
                { component: ['BEGIN (Pà)'] },
                { component: ['IN (URL)'] }
            ]
        },
      ...
      <p>Informació relativa a interaccions amb recursos</p>\
        ...
        filter:{component:['IN (Eina ext LTI)']}
            ...
            srcCSS: "",
            mode: WIDGET_CODE_SNIPPET,
            snippet: "var canvas = document.createElement('canvas');\
...
function navigatorInit(text, field)
{
  ...
  {
    let dataNavObj = {}, dataNav = [];
    let navigator = document.getElementById("menu-navigator");
    dashb.msldb.logs.forEach(
      item=>{
    ...
    let stStr = '\
    <form id="nav_' + field + '" onsubmit="return false;">\
    ...
    <select style="max-width:250px" onchange="navigatorFilter(this.selectedIndex,this.options[this.selectedIndex].text,\'' + field + '\');">';
    ...
    stStr += '</select><button class="btn small" onclick="let sel = this.form.elements[1]; if (sel.selectedIndex<sel.options.length-1) {sel.selectedIndex+=1;sel.onchange();}"><i class="fas fa-chevron-right"></i></button>\
    </form>';
    //if (null != document.getElementById("nav_" + field))
...
{
    if (widget.visible) {
        document.getElementById("widgets").insertAdjacentHTML('beforeend', widget.evaluatedCSSHTML);
    }
...
function globalFilter() {
    ...
    let filterList = ['IN', 'CONTAINS', 'BEGIN WITH', 'END WITH', 'BETWEEN', 'BETWEENEQ', 'EQUAL TO', 'NOTEQUAL TO', 'LESS THAN', 'LESSEQ TAHN', 'GREATER THAN', 'GREATEREQ THAN'];
    ...
    <form onsubmit="return false;">\
      ...
      <div>\
        <select>' + fieldsList.map(field => { return '<option>' + field + '</option>' }) + '</select>\
        &nbsp;\
        <select>' + filterList.map(filter => { return '<option>' + filter + '</option>' }) + '</select>\
        &nbsp;\
        <select>' + typeList.map(type => { return '<option>' + type + '</option>' }) + '</select>\
      </div>\
      <div>\
...
function dateFilter() {
  ...
  dashb.modal.headerContent = '<i class="fas fa-calendar-alt"></i>&nbsp;Date filter';
...
function editWidget(id) {
    ...
    <form onsubmit="return false;">\
      <p>Title</p>\
      <div>\
        <input name="title" type="text" value="' + widget._title + '">\
      </div>\
      <p>Size</p>\
      <div>\
        <input name="size" type="number" value="' + widget._size + '">\
      </div>\
      <p>Template</p>\
      <div id="editor-template" class="codemirror-editor">\
        <textarea name="codeditor-template" id="codeditor-template" style="display: none;">' + widget._html + '</textarea>\
      </div>\
      <p>JavaScript Snippet</p>\
      <div id="editor-js" class="codemirror-editor">\
        <textarea name="codeditor-js" id="codeditor-js" style="display: none;">' + widget._snippet + '</textarea>\
      </div>\
      <div>\
        <button onclick="updateWidget(\'' + id + '\', this.form);">Update widget</button>\
        <button onclick="deleteWidget(\'' + id + '\');">Delete widget</button>\
      </div>\
    </form>';
    ...
    let editorTemplate = CodeMirror.fromTextArea(document.getElementById("codeditor-template"), {
        lineNumbers: true,
        mode: "htmlmixed"
    });

    // Initialize mirror code editor
    let editorJS = CodeMirror.fromTextArea(document.getElementById("codeditor-js"), {
        lineNumbers: true,
        mode: "htmlmixed"
    });
...
function schema(item) {
    ...
    let fields = {
        fullDate: item[0],
        yearMonthDay: item[0].substr(6, 4) + "" + item[0].substr(3, 2) + "" + item[0].substr(0, 2),
        yearMonthDayHourMinute: item[0].substr(6, 4) + "" + item[0].substr(3, 2) + "" + item[0].substr(0, 2) + "" + item[0].substr(11, 2) + "" + item[0].substr(14, 2),
        timestamp:Date.parse(item[0].substr(6, 4) + "-" + item[0].substr(3, 2) + "-" + item[0].substr(0, 2) + "T" + item[0].substr(11, 2) + ":" + item[0].substr(14, 2) + ":00")/1000,
        user: item[1],
        name: item[1].split(" ")[0],
    ...
    fields.fullName = fields.middleName + " " + fields.lastName + ", " + fields.name;
...
function filters() {
    return {
        'IN': { regex: /^IN [(](.*)[)]/gm, fn: (r, v) => { return r.toLowerCase() == v.toLowerCase(); } },
        'CONTAINS': { regex: /CONTAINS [(](.*)[)]/gm, fn: (r, v) => { return -1 < r.toLowerCase().indexOf(v.toLowerCase()); } },
        'BEGIN': { regex: /BEGIN [(](.*)[)]/gm, fn: (r, v) => { return 0 == r.toLowerCase().indexOf(v.toLowerCase()); } },
        'END': { regex: /END [(](.*)[)]/gm, fn: (r, v) => { return (r.length - v.length) == r.toLowerCase().indexOf(v.toLowerCase()); } },
        'BETWEEN': { regex: /BETWEEN [(](.*)[)]/gm, fn: (r, v1, v2) => { return (r > v1) && (r < v2); } },
        'BETWEENEQ': { regex: /BETWEENEQ [(](.*)[)]/gm, fn: (r, v1, v2) => { return (r >= v1) && (r <= v2); } },
        'EQUAL': { regex: /== [(](.*)[)]/gm, fn: (r, v) => { return (r == v); } },
        'NOTEQUAL': { regex: /!= [(](.*)[)]/gm, fn: (r, v) => { return (r != v); } },
        'LESS': { regex: /< [(](.*)[)]/gm, fn: (r, v) => { return (r < v); } },
        'LESSEQ': { regex: /<= [(](.*)[)]/gm, fn: (r, v) => { return (r <= v); } },
        'GREATER': { regex: /> [(](.*)[)]/gm, fn: (r, v) => { return (r > v); } },
        'GREATEREQ': { regex: /> [(](.*)[)]/gm, fn: (r, v) => { return (r >= v); } }
    };
}
...
function renderDateFilter()
{
  ...
  if ((m = regex.exec(dashb.msldb.widgetFilter.yearMonthDay)) !== null) { 
    ...
    dateFilter.innerHTML = YMDToDate(values[0]).toLocaleDateString() + " - " + YMDToDate(values[1]).toLocaleDateString();
  }
}
...
function getDateFilter()
{
  ...
  if ((m = regex.exec(dashb.msldb.widgetFilter.yearMonthDay)) !== null) {
    let values = m[1].split(',');
    values[0] = values[0].substr(0, 4) + "-" + values[0].substr(4, 2) + "-" + values[0].substr(6, 2);
    values[1] = values[1].substr(0, 4) + "-" + values[1].substr(4, 2) + "-" + values[1].substr(6, 2);
    return values;
  }
  return []
}
...
{
  dashb.msldb.widgetFilter.yearMonthDay = ['BETWEENEQ (' + from.replace(/-/g,"") + ',' + to.replace(/-/g,"") + ')'];
  renderDateFilter();
  dashb.modal.close();
  reRenderDashboard();
}

window[addEventListener ? 'addEventListener' : 'attachEvent'](addEventListener ? 'load' : 'onload', loadApp);