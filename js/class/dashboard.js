...
class Dashboard
{
    ...
    init()
    {
        ...
        // render css
        document.head.insertAdjacentHTML("beforeend", "<style type=\"text/css\">"+this._gobalCfg.css+"</style>");
    }
    ...
    readFromFile(e, callbackLoad)
    {
        ...
        reader.onload = function(e)
        {
            ...
            self._msldb._schema = eval(dashbObj.db.schema.replace(/function .*()/g, 'item=>'));
            self._msldb._filters = eval(dashbObj.db.filters.replace(/function .*()/g, '()=>'));
    ...
    renderWidget(widget, callbackJS, callbackText)
    {
        ...
        if (WIDGET_CODE_SNIPPET === widget.mode)
        {
            ...
            if (Array.isArray(widget.filter))
            {
                widget.filter.forEach(
                    filter =>
                    {
                        ...
                        let lvs = this._msldb.labelsAndValues(widget.field, widget.sortBy, widget.order, widget.limit, widget.calcFn);
            ...
            else
            {
                ...
                widget.data = this._msldb.labelsAndValues(widget.field, widget.sortBy, widget.order, widget.limit, widget.calcFn);
            }
