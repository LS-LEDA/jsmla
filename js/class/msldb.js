...
class MoodleStandardLogsDataBase
{
    ...
    constructor(mdlbConfig)
    {
        ...
        /** @type {object} */
        this._globalFilter = (undefined!==mdlbConfig.globalFilter)?mdlbConfig.globalFilter:{};

        /** @type {object} */
        this._widgetFilter = (undefined!==mdlbConfig.widgetFilter)?mdlbConfig.widgetFilter:{};
    ...
    readLogFromFile(e, callbackLoad, callbackProgress)
    {
        ...
        {
            JSON.parse(e.target.result)[0].forEach(
                item =>
                {
     ...
     * @return {array} The countified labels value.
     */
    labelsCountGroup(field, groupField = 'fullDate', sortBy = 'value', order = 'DESC', limit = undefined)
    {
        let labels = {};
        let newLabels = new Array();
     ...
     * @return {array} The countified labels value.
     */
    labelsLastInteraction(field, groupField = 'fullDate', sortBy = 'value', order = 'DESC', limit = undefined)
    {
        let labels = {};
        let newLabels = new Array();
        ...
        this._logs.forEach(function(obj) {
            if (labels[obj[field]] < obj[groupField] || undefined === labels[obj[field]])
            {
     ...
     * @return {array} The labels value.
     */
    labelsAndValues(field, sortBy = 'value', order = 'DESC', limit = undefined, calcFn = {fn:"count", field:undefined})
    {
        let lav = {};
        ...
        if (undefined !== field)
        {
            ...
            switch (calcFn.fn)
            {
                ...
                case "countgroup":
                    labels = this.labelsCountGroup(field, calcFn.field, sortBy, order, limit);
                    break;
                // group by field and set values to: last connection
                case "lastconnection":
                    labels = this.labelsLastInteraction(field, calcFn.field, sortBy, order, limit);
                    break;
            }
